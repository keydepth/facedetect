...
img_file_name_list=os.listdir("./face_scratch_image0/")
print(len(img_file_name_list))

for i in range(len(img_file_name_list)):
    n=os.path.join("./face_scratch_image0",img_file_name_list[i])
    img = cv2.imread(n)
...
for j in range(0,len(img_file_name_list)-1):
    n=os.path.join("./face_scratch_image0/",img_file_name_list[j])
    img = cv2.imread(n)
    b,g,r = cv2.split(img)
    img = cv2.merge([r,g,b])
    X_train.append(img)
...
img_file_name_list=os.listdir("./test_image0/")
print(len(img_file_name_list))

for i in range(len(img_file_name_list)):
    n=os.path.join("./test_image0",img_file_name_list[i])
    img = cv2.imread(n)
    if isinstance(img,type(None)) == True:
        img_file_name_list.pop(i)
        continue
...
for j in range(0,len(img_file_name_list)):
    n=os.path.join("./test_image0",img_file_name_list[j])
    img = cv2.imread(n)
    b,g,r = cv2.split(img)
    img = cv2.merge([r,g,b])
    X_test.append(img)
...
if False:
	...
	model = Sequential()
	model.add(Conv2D(24, 3, padding='same', input_shape=(64, 64, 3)))
...
#モデルを保存
model.save("my_model-m2-epoch100.h5")
